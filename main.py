...
for file in os.listdir('data/one_plate'):
    ...
    data.drop(data[data.Light < np.average(data.Light) - 1.3 * (np.amax(data.Light) - np.average(data.Light))].index, inplace=True)
    ...
    # print(angle)
    popt, pcov = curve_fit(fitfunction, angle, data.Light, maxfev=500000, p0=[1.5, 0, np.average(data.Light), np.amax(data.Light) - np.average(data.Light)])
    ...
    ax.plot(angle, light, color='red')
    m = 0
    for j in range(1,len(light)-1):
        if light[j - 1] < light[j] > light[j + 1] and np.amin(light[peaks[m-1]:j]) < np.average(light) and sum([1 for x in light[peaks[m-1]:j] if x < np.average(light)]) / len(light[peaks[m-1]:j]) > 0.3 and light[j] > np.average(light) and light[j] > np.average(light) + 0.1 * (np.amax(light) - np.average(light)):
            #print(yum)
            peaks[m] = j
...
print(np.average(nny))
#thick = np.array(thick)
#thickerr = np.array(thickerr)
#print('For one plate the optimal thickness value and the erorr in it (in meters) are given by:')
...
for file in os.listdir('data/two_plates'):
    ...
    for j in range(len(nans) - 1):
        angle[nans[j]:nans[j+1]] = np.linspace(angle[nans[j]], angle[nans[j+1]], len(angle[nans[j]:nans[j+1]]))
    # print(angle)
    popt, pcov = curve_fit(fitfunction, angle, data.Light, maxfev=500000, p0=[2e-3, 0, statistics.median(data.Light), np.amax(data.Light) - statistics.median(data.Light)])

    plt.plot(angle, data.Light, color='red', label='Data')
    cal = np.zeros(len(angle))
...
thick = np.array(thick)
thickerr = np.array(thickerr)
print('For two plates the optimal thickness value and the erorr in it (in meters) are given by:')
print(np.sum(thick / thickerr ** 2) / np.sum(1 / thickerr ** 2))
print(np.sqrt(1 / np.sum(1 / thickerr ** 2)))
